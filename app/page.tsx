    ...
    <div className="flex flex-col min-h-screen pb-32 pt-8">
      ...
      <header className="px-6 mb-8 flex justify-between items-end">
        ...
        <div className="flex -space-x-3 mb-1">
          <div className="w-8 h-8 rounded-full border-2 border-navy-950 bg-gold-400 flex items-center justify-center text-[10px] font-bold text-navy-950">
            OM
          </div>
        </div>
      </header>
                ...
                {filteredBooks.map((book, idx) => (
                  <Link key={book.id} href={`/book/${book.id}`}>
                    ...
                    >
                      ...
                      <ChevronRight
                        size={20}
                        className="text-slate-600 group-hover:text-gold-500 transition-colors"
                      />
                    </motion.div>
                  </Link>
                ))}
...
function StatsCard({
  ...
  return (
    <div
      className={`flex flex-col items-center justify-center p-3 rounded-2xl ${color} border border-slate-800/50 shadow-inner backdrop-blur-sm group hover:scale-105 transition-transform cursor-pointer`}
    >
      <div className="mb-1 opacity-60 group-hover:scale-110 transition-transform">
        {icon}
      </div>
      <span className="text-xl font-bold font-sans tracking-tight">{count}</span>
      <span className="text-[10px] uppercase opacity-40 tracking-wider mt-0.5">
        {label}
      </span>
    </div>
  );
    ...
    <Link href={href} className="flex flex-col items-center gap-1 group">
      <div
        className={`p-2.5 rounded-2xl transition-all ${
          active
            ? "bg-gold-500/15 text-gold-500 shadow-md shadow-gold-500/10 ring-1 ring-gold-500/30"
      ...
      <span
        className={`text-[10px] font-bold font-sans uppercase tracking-[0.2em] transition-colors ${
          active ? "text-gold-500" : "text-slate-600"
        }`}
      >
        {label}
      </span>
